...
| **Дневник** | Личные записи по дням: текст + фото и видео (в объектном хранилище), навигация по датам, просмотр записей за любую дату |
| **Публикация каталога** | Новые блюда, упражнения и тренировки создаются как приватные черновики; в общий каталог («Все») попадают после кнопки «Опубликовать» на карточке |
| **Уведомления и напоминания** | Web Push (браузер/PWA, системные уведомления даже при закрытом сайте) через VAPID; настраиваемые напоминания по расписанию — интервал или фиксированные времена, дни недели, таймзона, и условие‑подавление («не напоминать про воду, если норма выполнена»; «про таблетки — если ещё не выпил»). Доставку раз в минуту проверяет воркер |
| **Калории и блюда** | Цели БЖУ/ккал, ручной ввод и выбор блюда, разделы «мои / все / избранное», рецепты, фото (S3/MinIO), оценки 1–5 и комментарии, добавление в рацион, сжигание калориями (ручное или MET по весу), дневник за день и история. **Составные блюда**: блюдо можно собрать из других блюд-ингредиентов с граммовкой — КБЖУ считается автоматически |
...
| **Дашборд** | Настраиваемый из «кубиков»‑виджетов (вода, калории, вес, давление, БЖУ, тренировки, таблетки, быстрые действия): показать/скрыть и переупорядочить, выбор сохраняется (новые виджеты появляются автоматически); анимированное появление, стиль с маскотом‑кабаном |
| **Интерфейс** | Светлая и тёмная темы (переключатель + запоминание выбора, по умолчанию — как в системе), профессиональный набор иконок (lucide), адаптивная вёрстка: на телефоне навигация сворачивается в меню‑бургер, карточки — в одну колонку |
...
- `CALENDAR_ENC_KEY` — (необязательно) отдельный ключ шифрования для хранимых
  паролей приложения CalDAV (Яндекс.Календарь). Пусто — ключ выводится из
  `JWT_SECRET`. Пользователь подключает календарь в «Настройках», указывая
  **пароль приложения** Яндекса ([Яндекс ID → Пароли приложений](https://id.yandex.ru/security/app-passwords),
  тип «Календарь CalDAV»), а не основной пароль. Раздел GTD → «Календарь»
  синхронизируется в обе стороны; фоновая синхронизация — в воркере раз в ~10 мин.
...
| GET/POST/PUT/DELETE | `/diary/day?date` · `/diary/entries…` · `/diary/media-upload-url` | Дневник по дням + вложения фото/видео |
| PUT/DELETE | `…/{id}/publish` (dishes, exercises, workouts) | Опубликовать / снять с публикации |
| GET/POST | `/push/key` · `/push/subscribe` · `/push/unsubscribe` · `/push/test` | Web Push: ключ VAPID, подписка устройства, тест |
| GET/POST/PUT/DELETE | `/reminders…` | Настраиваемые напоминания (расписание + условия) |
...
│     ├─ diary               # дневник по дням с фото/видео вложениями
│     ├─ push reminders      # web push (VAPID) + напоминания по расписанию
