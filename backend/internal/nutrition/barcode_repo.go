...
const barcodeCols = `barcode, name, brand, image_url,
	kcal_per_100g::float8, protein_per_100g::float8, fat_per_100g::float8, carbs_per_100g::float8, fiber_per_100g::float8,
...
func scanBarcode(row pgx.Row) (*BarcodeProduct, error) {
	...
	if err := row.Scan(&p.Barcode, &p.Name, &p.Brand, &p.ImageURL,
		&p.Per100.Kcal, &p.Per100.Protein, &p.Per100.Fat, &p.Per100.Carbs, &p.Per100.Fiber,
...
func (r *BarcodeRepo) Put(ctx context.Context, p BarcodeProduct) error {
	const q = `
		INSERT INTO food_barcodes (barcode, name, brand, image_url, kcal_per_100g, protein_per_100g,
			fat_per_100g, carbs_per_100g, fiber_per_100g, serving_grams, source, fetched_at)
		...
		ON CONFLICT (barcode) DO UPDATE SET
			...
			fat_per_100g=EXCLUDED.fat_per_100g, carbs_per_100g=EXCLUDED.carbs_per_100g, fiber_per_100g=EXCLUDED.fiber_per_100g,
	...
	_, err := r.db.Pool.Exec(ctx, q, p.Barcode, p.Name, p.Brand, p.ImageURL,
		p.Per100.Kcal, p.Per100.Protein, p.Per100.Fat, p.Per100.Carbs, p.Per100.Fiber, p.ServingGrams, p.Source)
	return err
}
